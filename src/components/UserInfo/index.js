...
const UserInfo = (props) => {
  ...
  const [uvaHandle, setUvaHandle] = useState(localStorage.getItem("uvaHandle") || "");
  const [atcoderHandle, setAtcoderHandle] =  useState(localStorage.getItem("atcoderHandle") || "");
  ...
  const handleSubmit = async () => { 
    ...
    if(codeforcesUserExists && uvaUserExists && atcoderUserExists) {
      props.onClose();
    }
  ...
  return (
    <Modal onClose={props.onClose}>
      <Paper className={classes.Paper}>
        ...
        />
        <LoadingButton loading={isLoading} variant="outlined" className={classes.FormButton} onClick={handleSubmit}>Save</LoadingButton >
      </Paper>
    </Modal>
  )
}
